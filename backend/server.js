...
app.get("/getAllNotes", express.json(), async (req, res) => {
    try {
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
          ...
          // Find all notes with posted by the user
          const collection = db.collection(COLLECTIONS.notes);
          ...
          if (!data) {
            return res
              .status(404)
              .json({ error: "Unable to find note with given user." });
          }
          const dataArray = await data.toArray();
          res.status(200).json({ response: dataArray });
        });
      } catch (error) {
        res.status(500).json({ error: error.message });
      }
    ...
    try {
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
          ...
          if(data.username != decoded.username){
            res.json({ response: "You are not allowed to delete other people's posts" });
          }
...
// Edit an existing note given the id
    ...
    try {
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
          ...
          if (!data) {
            ...
            const modify = await collection.updateOne({
                username: decoded.username,
                _id: new ObjectId(noteId)
            },
            {
